...
public class ProcessAnnotation {
	 private void sanitizerObject(Object object) throws Exception {
	        Class<?> clazz = object.getClass();//present for class,interface to get type object such as  construc ot,field,method
	        for (Method method : clazz.getDeclaredMethods()) {//class must have constructor
	            // Kiểm tra nếu object chứa một method có đính kèm @Sanitizer annotation
	            // Thì thực thi hàm đó
	            if (method.isAnnotationPresent(Sanitizer.class)) {
	                method.setAccessible(true);//ignore domain public,private,.. compel
